...
if __name__ == '__main__':
    ...
    #classes = [[-10000000, 0], [1, 20], [21, 40], [41, 60], [61, 80], [81, 150000000]]
    #pielbs = ['$<$ 0 trend'] + ["-".join([str(clas) for clas in classe]) for classe in classes[1:-1]] + ['$>$ 80']
    ...
    for src in srcs:
        ...
        for t0 in ts0:
            ...
            #drrs = xr.DataArray(out_drrs, dims=['lat', 'lon', 'ts0'], coords=dict(lon=(["lon"], lons), lat=(["lat"], lats), ts0=(["ts0"], ts0)))
    ...
    cb = fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax[:], extend='both', orientation='horizontal', label='FDT [years]', shrink=0.75)
    cb.set_ticklabels(ncols)   # [0, 10, 20, 30, 40, 50, 60, 70, 80]
    cb2 = fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), ax=ax2[:], extend='both', orientation='horizontal', label='FDT [years]', shrink=0.75)
    cb2.set_ticklabels(ncols)  # [0, 10, 20, 30, 40, 50, 60, 70, 80]
